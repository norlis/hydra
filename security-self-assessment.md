...
* **Gossip decompression cap**: node metadata received over gossip is zlib-compressed; decompression is capped at 1 MiB (`decodeMetaMaxBytes`, `internal/cluster/memberlist.go`), so a malicious cluster member cannot exhaust memory with a zlib bomb (~1000:1 expansion). A truncated payload fails JSON decoding and falls back to a stub node. Covered by unit tests (`internal/cluster/memberlist_test.go`).
